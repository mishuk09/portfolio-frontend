...
const MMUTasks = () => {
    ...
    const saveTimeToDatabase = async (time, status) => {
        try {
            const response = await fetch("https://mmu-task-backend.onrender.com/api/save-time", {
                method: "POST",
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch("https://mmu-task-backend.onrender.com/api/add-task", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    return (
        <>
            ...
            <h1 className="text-2xl md:text-3xl text-gray-800 pt-10 bg-gray-100 font-bold text-center">Add Time & Task</h1>
